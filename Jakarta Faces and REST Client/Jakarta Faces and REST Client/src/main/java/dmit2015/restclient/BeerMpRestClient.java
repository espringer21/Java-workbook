...
public interface BeerMpRestClient {
    ...
    String DOCUMENT_URL = "/beer";

    @POST
    @Path(DOCUMENT_URL + ".json")
    JsonObject create(Beer newBeer);

    @GET
    @Path(DOCUMENT_URL + ".json")
    LinkedHashMap<String, Beer> findAll();

    @GET
    @Path(DOCUMENT_URL + "/{key}.json")
    Beer findById(@PathParam("key") String key);

    @PUT
    @Path(DOCUMENT_URL + "/{key}.json")
    Beer update(@PathParam("key") String key, Beer updatedBeer);
